...
import com.cnLive.common.webSocket.Service.UserService;
import com.cnLive.common.webSocket.pojo.Users;
import com.cnLive.common.webSocket.utils.CreateId;
...
public class LiveRoom {
    ...
    private static Logger logger = LoggerFactory.getLogger(LiveRoom.class);
    ...
    public static Boolean openRoom(Users liver, Integer roomId, Session session) {
        ...
        logger.info("putMap:"+roomId+"||"+liveRoom);
    ...
    public void inRoom(Session session, Users user_login) {
        ...
        logger.info("Seesion:"+session+"||"+"User:"+user_login);
        if (user_login != null) {
            //已经登陆
            Set<Session> socketusergroup;
            if (this.getSocketsToUserMap().containsKey(user_login.getUserId())) {
                //该用户已经在该直播的在线列表中
