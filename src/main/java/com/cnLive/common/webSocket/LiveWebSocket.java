...
public class LiveWebSocket {
    private static Logger logger = LoggerFactory.getLogger(LiveWebSocket.class);
        ...
        if("rtc".equals(grop)){
            switch (type) {
                case "start_live"://主播启动直播
                    logger.info("liveRomm  user:" + user_login+"||roomId:"+roomId+"||session:"+session);
...
//                        logger.info(session.getId() + "-------" + liverSession.getId());
    ...
    public void onError(@PathParam(value = "roomId") Integer roomId, Session session, Throwable error) {
        ...
        logger.info("error:"+error.getCause());
